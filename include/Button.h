...
public:
    ...
    */
    Button(SDL_FRect rect, uint32_t color=0xFFFFFFFF, std::shared_ptr<SDL_Texture> texture=nullptr, uint32_t border_color=0xFFFFFFFF, uint32_t border_width=0, std::shared_ptr<TTF_Font> font=nullptr, std::string text="", uint32_t text_color=0xFFFFFFFF, std::function<void()> on_click=[](){}, std::function<void()> on_hover=[](){});
...
public:
    ...
    std::shared_ptr<TTF_Font> _font; /**< The button font. */
    std::string _text;              /**< The button text. */
    uint32_t _text_color;           /**< The button text color. */
private:
